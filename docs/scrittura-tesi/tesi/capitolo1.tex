...
\begin{enumerate}
    \item \textbf{Sviluppare un'architettura efficiente per CIFAR-10}: Implementare Mobile\-Net\-ECA combinando blocchi Inverted Residual con meccanismi di attenzione ECA.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{figure/ECA_Module.png}
    \caption{Schema del modulo ECA.}
    \label{fig:eca_module}
\end{figure}
\begin{table}[h]
\centering
...
\midrule
\textbf{MobileNetECA (ours)} & \textbf{92.47\%} & \textbf{0.077M} & \textbf{11M} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Grid search sistematica}: Esplorazione di 210 configurazioni per identificare empiricamente gli iperparametri ottimali.
    
    \item \textbf{Analisi quantitativa ECA}: Ablation study per quantificare il contributo dell'attenzione (+1.16\%).
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|c|}
        \hline
        \textbf{Architettura} & \textbf{Parametri} & \textbf{Acc. CIFAR-10} & \textbf{Note} \\
        \hline
        ResNet-18 & 11.2M & $\sim$95\% & Standard de facto \\
        MobileNetV2 (0.5x) & 0.8M & $\sim$90\% & Molto leggera ma meno accurata \\
        GhostNet (0.5x) & 0.6M & $\sim$91\% & Ottimo trade-off \\
        \textbf{MobileNetECA (Ours)} & \textbf{0.07M} & \textbf{92.5\%} & \textbf{State-of-the-Art Efficienza} \\
        \hline
    \end{tabular}
    \caption{Confronto preliminare tra efficienza e accuratezza. Il nostro modello (Ours) punta a massimizzare il rapporto Acc/Params.}
    \label{tab:pareto_intro}
\end{table}
