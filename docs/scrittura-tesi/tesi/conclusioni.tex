...
\begin{enumerate}
    ...
    \item \textbf{Grid search sistematica}: Esplorazione di 210 configurazioni di iperparametri attraverso un approccio two-phase (screening rapido + refinement), identificando empiricamente la combinazione ottimale di learning rate, width multiplier e weight decay.
    
    \item \textbf{Training finale}: Allenamento del modello ottimizzato per 200 epoche con SGD+momentum e Cosine Annealing schedule, raggiungendo un'accuratezza finale di \textbf{92.47\%} sul test set CIFAR-10.
    
    \item \textbf{Validazione sperimentale}: Confronto con modelli SOTA della letteratura e ablation study per quantificare il contributo dell'attenzione ECA (+1.16\% accuracy con overhead < 2\% parametri).
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Accuratezza}: 92.47\% su CIFAR-10 test set
    \item \textbf{Parametri}: 76,600 (~13× meno di MobileNetV2 ×0.5)
    \item \textbf{FLOPs}: ~11M MACs (~4× meno di MobileNetV2 ×0.5)
    \item \textbf{Dimensione}: < 300 KB (fit in L2 cache)
