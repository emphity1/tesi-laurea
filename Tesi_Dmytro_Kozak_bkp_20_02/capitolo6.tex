...
\begin{table}[h]
    \centering
    \caption{Risultati dell'Ablation Study. Ogni riga rappresenta l'aggiunta di una componente rispetto alla precedente. L'accuratezza di test è misurata una sola volta sul modello selezionato tramite early stopping sulla validazione.}
    \label{tab:ablation}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{l l c c c c}
        ...
        \midrule
        A & MobileNetV2-Micro (Baseline) & 91.65\% & 92.66\% & $\sim$78k & Optimized Stride/Layers \\
        B & MobileNetECA (A + ECA) & 92.17\% & 92.58\% & $\sim$78k & +0.52\% Test Acc \\
        C & MobileNetECA-Rep (B + Rep) & 92.43\% & 93.84\% & $\sim$77k\textsuperscript{*} & +0.26\% Test Acc \\
        D & MobileNetECA-Rep-AdvAug (Final) & \textbf{93.23\%} & \textbf{93.80\%} & \textbf{76.6k}\textsuperscript{*} & \textbf{+0.80\% Test Acc} \\
        \bottomrule
    \end{tabular}
    }
    \vspace{0.2em}
    \small{\textsuperscript{*}Parametri in modalità deploy (kernel fusi). In training: $\sim$85k.}
\end{table}
...
Il modello con Advanced Augmentation mostra una crescita dell'accuratezza più lenta nelle prime epoche rispetto alla baseline. Questo è atteso: le immagini "tagliate" o distorte sono più difficili da classificare. Tuttavia, nel lungo periodo (epoche $>150$), questa difficoltà costringe la rete ad apprendere feature più robuste e generalizzabili, portando al picco finale del 93.23\%.
...
\section{Analisi del Modello Finale (MobileNetECA-Rep-AdvAug)}
...
In letteratura sono presenti approcci efficienti recenti come GhostNet~\cite{han2020ghostnet}, che raggiunge il 93.38\% con 0.43M parametri applicando il modulo Ghost a ResNet-56, e MobileNetV3-Small~\cite{howard2019searching}, che ottiene il 92.97\% con 2.54M parametri. Il nostro modello si distingue da entrambi per l'ordine di grandezza inferiore di parametri (\textbf{0.08M} vs 0.43M--2.54M), mantenendo un'accuratezza competitiva (93.23\%).
...
\begin{figure}[h]
    ...
    \caption{Efficiency Frontier: Accuratezza vs Numero di Parametri (scala logaritmica). Il nostro modello (stella rossa) si trova nettamente a sinistra (meno parametri) rispetto a modelli con accuratezza simile come ResNet-32 o GhostNet.}
...
Con un throughput superiore a \textbf{144 FPS} su una CPU generica, il modello garantisce ampiamente prestazioni real-time senza necessità di acceleratori hardware dedicati. Su microcontrollori performanti (es. STM32H7) o processori mobile (Snapdragon), ci si attende una latenza ulteriormente ridotta, rendendo l'architettura ideale per droni, videosorveglianza e robotica a basso costo.
...
\paragraph{Nota Metodologica.}
La Grid Search è stata condotta sulla configurazione Baseline (A) senza Advanced Augmentation, raggiungendo un massimo di circa 91.4\%. I risultati finali (93.23\%) incorporano le tecniche di augmentation avanzata descritte nella Sezione 5.4, che forniscono il restante guadagno prestazionale.
...
In conclusione, \textbf{MobileNetECA-Rep-AdvAug} dimostra che è possibile ottenere prestazioni da "Server-Class" (93\%+) con un budget di risorse da "Microcontroller-Class", grazie alla sinergia tra depthwise convolution, attenzione ECA e ri-parametrizzazione.
