...
La soluzione proposta (visualizzata in Figura~\ref{fig:mobilenet_eca_rep_overview}), denominata \textbf{MobileNetECA-Rep}, è un'architettura ibrida che sintetizza tre paradigmi moderni di progettazione efficiente:
\begin{enumerate}
...
\begin{figure}[h]
    ...
    \caption[Architettura MobileNetECARep.]{Architettura MobileNetECARep. I blocchi azzurri rappresentano i tensori intermedi. Il dettaglio mostra il modulo RepInvertedResidual con integrazione di Efficient Channel Attention (ECA) e la struttura di convoluzione riparametrizzabile (RepConv).}
...
\begin{figure}[h]
    ...
    \caption[Visualizzazione della fusione dei kernel.]{Visualizzazione della fusione dei kernel. Il kernel $1 \times 1$ viene portato a $3 \times 3$ tramite zero-padding, mentre l'identità diventa un kernel $3 \times 3$ con 1 al centro e 0 altrove. La somma finale produce un unico kernel $3 \times 3$ equivalente.}
...
Abbiamo sostituito la classica ReLU con \textbf{GELU (Gaussian Error Linear Unit)} (confronto visivo in Figura~\ref{fig:gelu_vs_relu}).
...
\begin{figure}[h]
    ...
    \caption[Confronto tra ReLU e GELU.]{Confronto tra ReLU (blu) e GELU (gialla). La natura liscia e non monotona della GELU vicino allo zero favorisce un flusso dei gradienti più robusto.}
...
Un aspetto cruciale per l'ingegnizzazione del sistema consiste nella codifica della funzione di \textit{re-parametrizzazione}.
...
\begin{figure}[h]
    ...
    \caption[Confronto Standard Convolution vs Depthwise Separable.]{Confronto Standard Convolution vs Depthwise Separable (usata in MobileNet). La re-parametrizzazione permette di usare strutture complesse in training che collassano in efficienti depthwise/standard conv all'inferenza. L'immagine evidenzia la differenza fondamentale tra l'operazione spaziale densa e quella fattorizzata.}
    \label{fig:conv_comparison}
\end{figure}

Come contestualizzato anche visivamente in Figura~\ref{fig:conv_comparison}, questo passaggio è fondamentale: trasforma un grafo computazionale complesso (multi-branch) in una semplice sequenza di operazioni lineari, riducendo significativamente la latenza di inferenza su CPU~\cite{ding2021repvgg} e permettendo l'export verso formati ottimizzati come TFLite.
...
L'implementazione del blocco ECA è cruciale per mantenere l'efficienza. A differenza dei blocchi SE (Squeeze-and-Excitation) che usano due layer Fully Connected (FC) con riduzione di dimensionalità, ECA usa una convoluzione 1D adattiva (Figura \ref{fig:eca_detail}).

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{figure/ECA_block.jpg}
    \caption[Schema dettagliato del modulo ECA.]{Schema dettagliato del modulo ECA (Efficient Channel Attention). Viene eseguita una Global Average Pooling seguita da una convoluzione 1D con kernel adattivo e funzione sigmoidea.}
    \label{fig:eca_detail}
\end{figure}

\subsubsection{Perché ECA e non SE?}
In un modello con meno di 100k parametri totali, ogni parametro aggiuntivo ha un costo proporzionale molto elevato.
Un blocco SE standard per $C$ canali con fattore di riduzione $r$ aggiunge $\frac{2C^2}{r}$ parametri (due layer FC).
Il blocco ECA, al contrario, richiede un solo kernel di Conv1D con $k \approx 5$ parametri.

Per rendere concreto il confronto, consideriamo lo strato con il massimo numero di canali espansi nel nostro modello ($C = 208$, hidden\_dim dello stadio con $t=8$, $c=26$, width\_mult=$0.5$):

\begin{table}[h]
    \centering
    \caption{Confronto parametri aggiuntivi: SE Block vs ECA Block per $C=208$ canali.}
    \label{tab:se_vs_eca}
    \begin{tabular}{l c c}
        \toprule
        \textbf{Meccanismo} & \textbf{Parametri per blocco} & \textbf{Overhead su modello} \\
        \midrule
        SE ($r=4$)  & $2 \times 208 \times 52 = 21{,}632$ & $+28.2\%$ \\
        SE ($r=16$) & $2 \times 208 \times 13 = 5{,}408$  & $+7.1\%$ \\
        \textbf{ECA ($k=5$)} & \textbf{5}                   & $\mathbf{+0.007\%}$ \\
        \bottomrule
    \end{tabular}
\end{table}

Come mostrato nella Tabella \ref{tab:se_vs_eca}, anche con il fattore di riduzione più aggressivo ($r=16$), un singolo blocco SE aggiungerebbe oltre 5k parametri — circa il 7\% dell'intero modello. Con 12 blocchi Inverted Residual, l'impatto cumulativo renderebbe SE proibitivo. ECA, con soli 5 parametri per blocco (totale $\sim$80 parametri per l'intero modello), fornisce un'attenzione efficace a costo essenzialmente nullo.
...
\end{equation}
Con parametri ottimizzati per le basse risoluzioni di CIFAR-10: $b=12$, $\gamma=3$.
Questo approccio garantisce che ogni canale interagisca solo con i suoi $k$ vicini locali, preservando le relazioni tra canali adiacenti e riducendo drasticamente il numero di parametri (da $C^2$ per SE a $k$ per ECA).

\subsubsection{Implementazione PyTorch}
Di seguito l'implementazione completa del blocco ECA utilizzata nel nostro modello:

\begin{lstlisting}[language=Python, caption=Implementazione del blocco ECA (Efficient Channel Attention), basicstyle=\footnotesize\ttfamily]
class ECABlock(nn.Module):
    def __init__(self, channels, gamma=3, b=12):
        super().__init__()
        # Kernel adattivo: piu' canali -> kernel piu' grande
        t = int(abs((math.log(channels, 2) + b) / gamma))
        k = t if t % 2 else t + 1  # Forza dispari
    ...
    def forward(self, x):
        ...
        y = self.conv(y)                   # Conv1D locale
        y = self.sigmoid(y)                # Pesi [0, 1]
        y = y.transpose(-1,-2).unsqueeze(-1) # [B, C, 1, 1]
        return x * y.expand_as(x)  # Ricalibrazione
\end{lstlisting}

\subsubsection{Pipeline Operativa}
La sequenza di operazioni nel blocco ECA è la seguente:
\begin{enumerate}
    \item \textbf{Global Average Pooling}: Riduce le dimensioni spaziali $H \times W$ a $1 \times 1$, creando un descrittore globale per ogni canale.
    \item \textbf{Conv1D}: Applica il kernel adattivo lungo la dimensione dei canali, catturando le interazioni locali tra canali adiacenti.
    \item \textbf{Sigmoid}: Normalizza i pesi di attenzione nell'intervallo $[0, 1]$.
    \item \textbf{Scale}: Moltiplica element-wise le feature map originali per i pesi calcolati, amplificando i canali informativi e attenuando quelli ridondanti.
\end{enumerate}
...
\paragraph{Protocollo di Validazione}
Per garantire una valutazione rigorosa e priva di bias, il dataset di training originale (50.000 immagini) è stato suddiviso in un sottoinsieme di addestramento (45.000 immagini) e un insieme di validazione (5.000 immagini), utilizzando uno split deterministico con seed fisso. La selezione del modello migliore (early stopping) è stata effettuata esclusivamente sulla base dell'accuratezza di validazione. Il test set originale (10.000 immagini) è stato utilizzato una sola volta per la valutazione finale, evitando qualsiasi forma di information leakage.
...
\begin{figure}[h]
    ...
    \caption[Andamento del Learning Rate durante le 200 epoche di training.]{Andamento del Learning Rate durante le 200 epoche di training (Cosine Annealing). Il tasso di apprendimento decresce dolcemente seguendo una curva cosinusoidale.}
...
Il modello finale \textbf{MobileNetECA-Rep-AdvAug} presenta caratteristiche di efficienza notevoli, come riassunto nella Tabella \ref{tab:complexity_new}.

\begin{table}[h]
    \centering
    \caption{Confronto Complessità: MobileNetECA-Rep-AdvAug vs Baseline e Letteratura}
    ...
    \begin{tabular}{l c c}
        ...
        \midrule
        ResNet-20 & 0.27M & 41M \\
        MobileNetV2-Standard (0.5x) \textit{da SOTA} & 0.70M & 25.3M \\
        MobileNetV2-Micro (Nostra Baseline A) & 0.078M & 9.4M \\
        \textbf{MobileNetECA-Rep-AdvAug (Finale)} & \textbf{0.062M} & \textbf{9.4M} \\
        \bottomrule
    \end{tabular}
\end{table}

Con soli \textbf{62,194 parametri} in fase di inferenza a parità di costo computazionale residuo (\textbf{9.4 Milioni di FLOPs}), il nostro modello finale non solo è \textbf{più snello del 20\% rispetto alla nostra stessa Baseline A} originaria (78k parametri), ma risulta circa \textbf{11 volte più compatto} e quasi \textbf{3 volte più efficiente} rispetto a una MobileNetV2 Standard proveniente dalla letteratura (scalata a 0.5x, con $\sim$0.70M parametri e 25.3M FLOPs).
Il modello è inoltre oltre \textbf{4 volte più leggero} di una classica ResNet-20, rendendolo idoneo all'esecuzione in real-time anche su microcontrollori di fascia molto bassa (es. STM32 o ESP32) che operano con budget di potenza nell'ordine dei milliwatt.

\subsection{Decostruzione della Complessità: il Collasso dei Parametri}
Un aspetto che merita particolare attenzione è la discrepanza numerica tra i parametri della rete originaria Baseline (MobileNetV2-Micro, $\sim$78k parametri), i parametri della nostra architettura durante la fase di training ($\sim$85k) e l'inaspettato crollo finale a sole \textbf{62,194 unità} in fase di deploy. 
Come è possibile che un modello a cui sono stati \textit{aggiunti} rami paralleli e moduli di attenzione extra (ECA) risulti alla fine \textit{più leggero} del suo predecessore Vanilla?

La risposta risiede in due fenomeni matematici che collassano simultaneamente durante l'esecuzione della ri-parametrizzazione strutturale (\texttt{switch\_to\_deploy}):

\begin{enumerate}
    \item \textbf{La Fusione della Batch Normalization (BN)}: Nel modello Baseline, ogni strato convoluzionale è seguito da un layer di BN. In inferenza, PyTorch conserva i parametri della BN (peso $\gamma$, bias $\beta$, `running\_mean` e `running\_var`) come tensori computazionali separati. Nel nostro modello, come dimostrato formalmente nelle Equazioni 5.3 e 5.4, la BN viene matematicamente assorbita nei pesi ($W'$) e nei bias ($b'$) della convoluzione stessa. Questo elimina letteralmente migliaia di parametri lineari dal conteggio finale.
    \item \textbf{L'Unificazione dei Rami}: I rami $1\times1$, $3\times3$ e l'identità spariscono assieme a tutte le loro rispettive pre-computazioni e BatchNorm, convergendo in una singola e densa matrice $3\times3$, trasformando un tensore multi-struttura in un puro "peso + bias".
\end{enumerate}

Per dimostrare empiricamente questo collasso algebrico, un'ispezione layer-by-layer del nostro script di validazione mostra come, ad esempio, nel primo stadio di espansione della rete (il macro-blocco \texttt{features.1}, che raggruppa 4 blocchi Inverted Residual consecutivi espansi di un fattore 6), il conteggio dei parametri utili del modello passi da 35.394 a \textbf{29.260}, vaporizzando oltre 6.000 pesi non essenziali in un singolo colpo. Pur rappresentando quasi la metà della capacità computazionale dell'intero modello (fondamentale per estrarre le feature spaziali ad alta risoluzione), l'ottimizzazione interviene drasticamente alleggerendone l'impatto.
Questa riparametrizzazione strutturale rende \textbf{MobileNetECA-Rep-AdvAug} particolarmente efficiente per il deployment su dispositivi edge: il modello mantiene una complessa topologia multi-ramo durante il training per massimizzare l'apprendimento, per poi essere convertito in un'architettura estremamente lineare e snella in fase di inferenza.