...
class DropBlock(nn.Module):
    ...
    def forward(self, x):
        if not self.training or self.drop_prob == 0:
            return x
        gamma = self.drop_prob / (self.block_size ** 2) * x.size(2) * x.size(3) / ((x.size(2) - self.block_size + 1) * (x.size(3) - self.block_size + 1))
        mask = (torch.rand(x.size(0), x.size(1), x.size(2) - self.block_size + 1, x.size(3) - self.block_size + 1, device=x.device) < gamma).float()
        mask = mask.repeat(1, 1, self.block_size, self.block_size)
        mask = mask[:,:x.size(1),:x.size(2),:x.size(3)]
        mask = 1 - mask
        x = x * mask
...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, drop_prob=0.0):
        ...
        self.dropblock = DropBlock(block_size=5, drop_prob=0.3)
...
class ResNetLight(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.in_channels = 12  # Ridotto ulteriormente il numero di canali
        self.conv1 = nn.Conv2d(3, 12, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(12)
        self.layer1 = self._make_layer(12, 2, stride=1)  # Manteniamo 2 blocchi
        self.layer2 = self._make_layer(24, 2, stride=2)  # Ridotto a 24
        self.layer3 = self._make_layer(48, 2, stride=2)  # Ridotto a 48
        self.linear = nn.Linear(48, num_classes)  # Adeguato per abbinare l'output del layer finale
...
class DummyTrainer:
    def __init__(self, epochs=50, batch_size=512, base_lr=0.002, max_lr=0.025, weight_decay=1e-4, cutout_length=16, smoothing=0.1):
        self.epochs = epochs
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, data in enumerate(self.trainloader, 0):
                ...
                # Mixup
                inputs, targets_a, targets_b, lam = self.mixup(inputs, labels)
                ...
                loss.backward()
                self.optimizer.step()
                self.scheduler.step(epoch + i / len(self.trainloader))
        ...
        # Rimuove la prunatura prima di salvare il modello
        self.remove_pruning()
    ...
    def test(self, epoch):
        ...
        # Salva il modello se l'accuratezza è migliorata
        if accuracy > self.best_accuracy:
            self.best_accuracy = accuracy
            print(f'New best accuracy: {self.best_accuracy:.2f}%. Saving model...')
