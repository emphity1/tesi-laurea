...
class ResNetLight(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.in_channels = 32  # Aumentato da 16 a 32 canali
        self.conv1 = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(32)
        self.layer1 = self._make_layer(32, 1, stride=1)  # 1 blocco
        self.layer2 = self._make_layer(64, 1, stride=2)  # 64 canali e 1 blocco
        self.layer3 = self._make_layer(128, 1, stride=2)  # 128 canali e 1 blocco
        self.linear = nn.Linear(128, num_classes)  # Adattato per il layer finale
...
class WrappedModel(nn.Module):
    def __init__(self, model, num_classes, device):
        ...
        self.additional_layer1 = nn.Linear(example_output.size(1), 256)
        self.additional_layer2 = nn.Linear(256, 256)
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.additional_layer1(x))
        x = self.dropout(x)
        x = torch.relu(self.additional_layer2(x))
        x = self.dropout(x)
        x = self.output_layer(x)
        return x

class DummyTrainer:
    def __init__(self, model_path='/workspace/BESTMODELS/models/model_prova.pt', epochs=100, batch_size=1024, base_lr=0.001, max_lr=0.01, weight_decay=1e-4, cutout_length=16):
        self.epochs = epochs
        ...
        # Sblocca alcuni layer per il fine-tuning
        ...
        for param in self.model.additional_layer1.parameters():
            param.requires_grad = True

        for param in self.model.additional_layer2.parameters():
            param.requires_grad = True
        ...
        self.optimizer = optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), lr=self.base_lr, weight_decay=self.weight_decay)
