...
# Calcolo Parametri e MACs (Multiply-Accumulate Operations)
# Differenza FLOPs vs MACs:
# - FLOPs (Floating Point Operations): Conta ogni singola operazione (+, *, /).
# - MACs (Multiply-Accumulates): Conta l'operazione combinata "a + b * c", tipica delle convoluzioni.
# In genere: 1 MAC ≈ 2 FLOPs.
# Per l'hardware deep learning (NPU/GPU), i MACs sono la metrica più rilevante per stimare la latenza.

def get_macs(model, input_size=(1, 3, 32, 32)):
    try:
        from thop import profile
        input = torch.randn(input_size).to(dispositivo)
        macs, params = profile(model, inputs=(input, ), verbose=False)
        return macs
    except ImportError:
        print("Libreria 'thop' non trovata. Installa con `pip install thop` per il calcolo esatto dei MACs.")
        return 0

macs = get_macs(modello)
print(f"------ Metriche Computazionali ------")
print(f"Parametri: {formatta_numero(params)}")
if macs > 0:
    print(f"MACs: {formatta_numero(macs)}")
else:
    print("MACs: N/A (thop required)")
print("-------------------------------------")
...
print(f"\n{'='*70}")
print(f"MODELLO STANDARD 200 EPOCHS RUN")
print(f"{'='*70}\n")

for epoca in range(epoche):
    acc_train, avg_loss = allenamento()
    acc_valid = validazione()
    schedulatore.step()
    current_lr = ottimizzatore.param_groups[0]['lr']
    ...
    if acc_valid > best_acc:
        best_acc = acc_valid
        torch.save(modello.state_dict(), os.path.join(OUTPUT_DIR, 'best_model_standard.pth'))

    print(f'Epoca {epoca+1:03d}/{epoche} - Loss: {avg_loss:.4f} | Train: {acc_train:.2f}% | Val: {acc_valid:.2f}% (Best: {best_acc:.2f}%)')

# Salvataggio JSON
with open(os.path.join(OUTPUT_DIR, 'MobileNetECA_200_history.json'), 'w') as f:
    json.dump(history, f)
