...
_run_dir = os.path.join(_script_dir, "v3_results")
...
class ECABlock(nn.Module):
    # V3 (aka V1): gamma=3, b=12 (Custom Optimized for CIFAR10 low-res)
...
class RepInvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, use_eca=True):
        ...
        if use_eca:
            layers.append(ECABlock(hidden_dim)) # Custom V1 param
...
class MobileNetECARep_v3(nn.Module):
    def __init__(self, num_classes=10, width_mult=0.5):
        super(MobileNetECARep_v3, self).__init__()
...
BATCH_SIZE = 128
EPOCHS = 200
# FIXED: Using proven V1 values explicitly
LR = 0.05 
WEIGHT_DECAY = 5e-4
MOMENTUM = 0.9
...
# Modello
model = MobileNetECARep_v3(width_mult=0.5).to(device)
...
logging.INFO("Starting Training V3 (Reproducible)...")
...
for epoch in range(EPOCHS):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(model.state_dict(), os.path.join(_run_dir, "best_model_v3.pth"))
...
# Save stats
with open(os.path.join(_run_dir, "stats_v3.json"), "w") as f:
    json.dump(stats, f)

# Deploy Export
model_deploy = copy.deepcopy(model)
model_deploy.load_state_dict(torch.load(os.path.join(_run_dir, "best_model_v3.pth")))
model_deploy.deploy()
model_deploy.eval()
torch.save(model_deploy.state_dict(), os.path.join(_run_dir, "best_model_v3_deploy.pth"))
