...
OUTPUT_DIR = '/workspace/tesi-laurea/reports/eca_vs_eca_parametrized'
...
if __name__ == "__main__":
    ...
    trasformazione_train = transforms.Compose([
        ...
        transforms.RandomErasing(p=0.25, scale=(0.02, 0.33), ratio=(0.3, 3.3), value=0) # Cutout moderno
    ])
    ...
    trainset = datasets.CIFAR10(root='./data', train=True, download=True, transform=trasformazione_train)
    testset = datasets.CIFAR10(root='./data', train=False, download=True, transform=trasformazione_test)
    caricatore_train = DataLoader(trainset, batch_size=dimensione_batch, shuffle=True, num_workers=2, pin_memory=True)
    caricatore_test = DataLoader(testset, batch_size=dimensione_batch, shuffle=False, num_workers=2, pin_memory=True)

    # OUTPUT_DIR = '/workspace/tesi-laurea/reports/adv_aug_test'
    OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    print(f"\n{'='*70}")
    print(f"MODELLO ADVANCED AUGMENTATION (AutoAugment + Cutout)")
    print(f"Target Epochs: {epoche}") 
    print(f"{'='*70}\n")
    ...
    print(f"\n{'='*70}")
    print(f"MODELLO REPARAMETERIZED 200 EPOCHS RUN")
    print(f"Parametri Training: {formatta_numero(sum(p.numel() for p in modello.parameters()))}")
    print(f"{'='*70}\n")
    ...
    with open(os.path.join(OUTPUT_DIR, 'MobileNetECA_Rep_200_history.json'), 'w') as f:
        json.dump(history, f)
