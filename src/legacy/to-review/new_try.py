...
fattore_larghezza = 0.44  # Fattore per regolare la capacità del modello
dispositivo = 'cuda' if torch.cuda.is_available() else 'cpu'  # Uso di CUDA se disponibile
lr_scale = 1.46  # Valore di lr_scale utilizzato nel modello
# ============================================
...
class SEBlock(nn.Module):
    def __init__(self, channels, reduction=24):
...
class ECABlock(nn.Module):
    def __init__(self, channels, gamma=2, b=1, lr_scale=1.6):
        ...
        # Calcola la dimensione del kernel basandosi sul numero di canali
    ...
    def forward(self, x):
        # Riduce la dimensione spaziale a 1x1 per ogni canale
        y = self.avg_pool(x)
        # Cambia la dimensione per adattarsi alla convoluzione 1D
        y = y.squeeze(-1).transpose(-1, -2)  # Converte da (B, C, 1, 1) a (B, 1, C)
        y = self.conv(y)
        y = self.sigmoid(y)
        y = y.transpose(-1, -2).unsqueeze(-1)  # Torna a (B, C, 1, 1)
        # Scala il gradiente per stabilità
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, use_eca=False, use_se=False, lr_scale=1.6):
...
class MobileNetECA(nn.Module):
    def __init__(self, num_classes=10, width_mult=0.2, lr_scale=1.44):
        super(MobileNetECA, self).__init__()
        # Impostazioni per i blocchi
        block_settings = [
            # t, c, n, s, use_eca, use_se
            [1, 18, 1, 1, True, True],
            [4, 24, 3, 2, True, True],
            [8, 34, 4, 2, True, True],
            [8, 48, 2, 1, True, True],
        ...
        last_channel = max(int(128 * width_mult), 8)
        ...
        for t, c, n, s, use_eca, use_se in block_settings:
            output_channel = max(int(c * width_mult), 8)
            for i in range(n):
                stride = s if i == 0 else 1
                self.features.append(
                    InvertedResidual(input_channel, output_channel, stride, expand_ratio=t, use_eca=use_eca, use_se=use_se, lr_scale=lr_scale)
                )
...
modello_scriptato.save('/workspace/Dima/models/modello_alternativo.pt')
