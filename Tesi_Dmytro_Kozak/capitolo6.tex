...
In letteratura sono presenti approcci ultra-compatti basati su architetture ricorsive (che riutilizzano lo stesso filtro in più iterazioni), capaci di raggiungere $\sim$91\% con meno di 50k parametri.
Sebbene il nostro modello utilizzi un numero leggermente superiore di parametri ($\sim$76k), rimane ben al di sotto della soglia critica dei 100k, offrendo un guadagno di accuratezza sostanziale (+2.5\%) e una struttura feed-forward lineare molto più semplice da parallelizzare su hardware moderno rispetto alle architetture ricorsive.
...
\paragraph{Nota sulla Variabilità.}
La deviazione standard elevata ($\sigma > \mu$) è attribuibile alle prime iterazioni di warmup (JIT compilation e allocazione cache), che producono latenze anomale. Escludendo le prime 50 iterazioni, la distribuzione risulta stabile con valori concentrati intorno alla media.
