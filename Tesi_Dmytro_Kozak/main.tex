...
\titolo{Sviluppo di Architetture Neurali Ultra-Compatte tramite Attention Mechanisms, Reparameterization e Knowledge Distillation}
\autore{Dmytro Kozak}
\matricola{538593}
...
\clearpage
\begin{abstract}
    In questa tesi viene proposta un'architettura ibrida e ultra-compatta per la classificazione efficiente di immagini su dispositivi edge, che integra i principi di \textit{MobileNetV2}, \textit{Efficient Channel Attention (ECA)} e \textit{Structural Reparameterization}. 
    Al fine di mitigare la severa riduzione della capacità parametrica, il framework di addestramento è stato potenziato mediante tecniche avanzate di regolarizzazione, integrando la \textit{Knowledge Distillation} da un modello Teacher pre-addestrato e l'\textit{Exponential Moving Average (EMA)} dei pesi. 
    Il modello studiato e finalizzato, denominato \textit{MobileNetECA-Rep}, raggiunge un'accuratezza del 93.76\% sul dataset CIFAR-10 con un footprint di soli $\sim$62k parametri, dimostrando risultati competitivi con architetture nettamente superiori in termini di complessità complessiva e computazionale. 
    L'elaborato analizza rigorosamente le dinamiche di ottimizzazione, quantifica l'impatto architetturale con l'impiego capillare di \textit{ablation study} e valuta empiricamente la latenza di inferenza, testando e validando la fruizione dell'architettura in uno scenario di applicabilità \textit{real-time}.
\end{abstract}
\clearpage
